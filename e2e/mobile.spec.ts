...
test('stays compact and in bounds across every mobile page', async ({ page }) => {
  ...
  await expect(page.getByText('ZFS allocated', { exact: true })).toBeVisible()
  await expect(page.getByText('Logical blocks', { exact: true })).toBeVisible()
  await expect(page.getByText('Disk size', { exact: true })).toBeVisible()
  await expect(page.getByText('VM block volume · guest free space unavailable')).toBeVisible()
